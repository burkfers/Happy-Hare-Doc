...
#
# This file lives in the Happy-Hare-Doc repo, but installer/Kconfig itself lives in
# Happy-Hare - see HAPPY_HARE_SRC below (set for you by `make shots`, or point it at
# a checkout you already have). Everything else about how this works is unaffected
# by that split: it's still the real installer/Kconfig tree through the real
# menuconfig, headlessly, rendering any screen it can reach to a PNG:
#
...
# Kconfig from a Happy-Hare checkout fetched elsewhere (HAPPY_HARE_SRC - see
# the Makefile's 'fetch-source' target, or override it directly for a local
# checkout: HAPPY_HARE_SRC=/path/to/Happy-Hare make shots).
...
if not HAPPY_HARE_SRC or not os.path.isfile(os.path.join(HAPPY_HARE_SRC, 'installer', 'Kconfig')):
    raise SystemExit(
        "HAPPY_HARE_SRC must point at a Happy-Hare checkout (containing installer/Kconfig) - "
        "run via `make shots`, which fetches one and sets this for you, or set it yourself to "
        "an existing checkout for faster local iteration."
    )
INSTALLER = os.path.join(HAPPY_HARE_SRC, 'installer')
...
def hh_version():
    ...
    """
    path = os.path.join(HAPPY_HARE_SRC, 'extras', 'mmu', 'mmu_constants.py')
...
class Menuconfig:
    ...
    def __init__(self, cols=DEFAULT_COLS, rows=40, seed=DEFAULT_SEED, style=None,
        ...
        if self.pid == 0:                            # child
            ...
            os.chdir(HAPPY_HARE_SRC)                  # matches cwd when install.sh/make runs this for real
