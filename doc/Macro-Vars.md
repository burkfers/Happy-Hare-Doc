...
menuconfig help text behind each one, the same way as [Parameters](Parameters.md)
...
| `cooling_tube_position` | `35` mm | Distance from the nozzle tip to the start of the cooling tube |
| `cooling_tube_length` | `10` mm | Length of the cooling-move travel |
...
| Variable | Default | Description |
|---|---|---|
| `extruder_purge_speed` | `2` mm/s | Purge speed - as fast as possible without the extruder skipping steps (extruder current can also be raised in [`extruder_purge_current`](Parameters.md#tip-forming)) |
...
- [Parameters](Parameters.md) - the `mmu.cfg`/`mmu_parameters.cfg` config
