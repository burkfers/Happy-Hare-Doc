...
Driving this same harness by hand, interactively, rather than from a test file, is its own
page: [The Happy Hare Simulator](Dev-Simulator.md) (`make console`).
...
`make test` offers everything — **well over 900 tests** and growing, a couple of minutes
on a warm laptop. Expect to see:
...
```{.text .console-output}
Happy Hare tests - 900+ tests in ~30 files        times from last run (~ = reference, never run locally)

   1 [x] installer.test_build           1     0.0s
   2 [x] test_mmu_adc_compat           14     0.0s
   3 [x] test_mmu_bootup               34     2.3s
   …
   7 [x] test_mmu_console             148      37s
   …
  15 [x] test_mmu_nfc                  17     9.7s
   …
  30 [x] test_mmu_toolchange           20     0.9s

  selected: ~30 files - 900+ tests - ~1m42s last time
...
test/
  ...
  console.py        the interactive console (see Dev-Simulator.md)
...
The test files are grouped by what they're about — foundation (imports, config rendering,
bootup, profiles), filament handling (motion, toolchange, encoder, endless spool), NFC and
Spoolman, and presentation (LEDs, the console). Browse `test/test_mmu_*.py` directly for the
current list rather than trusting a table here — it only grows, and a stale count is worse
than none.

One file to know by name: `test_mmu_console.py` is one of the largest, at close to 150 tests
covering the [interactive console](Dev-Simulator.md)'s rendering and command dispatch —
worth a look as an example of testing something more presentation-heavy than the filament
model itself.
