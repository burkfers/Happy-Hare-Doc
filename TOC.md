...
| Page | Source | Status |
|---|---|---|
| `Understanding-Operation.md` | `wiki/Understanding-Operation.md` + current Box Turtle command output | **done** — rebuilt the legacy status walkthrough around a current four-gate Box Turtle snapshot. Covers the machine/selection state and the dynamically calculated preload, load and unload sequences from `MMU_STATUS SHOWCONFIG=1`, then cross-checks that report against the current `MMU_GATE_MAP`, `MMU_TTG_MAP` and `MMU_SENSORS` output. The old point-in-time status format, parameter names and version-specific prose were replaced rather than preserved; deeper map/sensor configuration remains on the existing Feature pages. Added first in the Operation navigation so readers learn how to inspect state before acting on it. |
